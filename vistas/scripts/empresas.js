...
function listar()
{
	tabla = $("#tbllistado").DataTable(
	{
		...
		"ajax":
				{
					url: '../ajax/empresas.php?op=listar',
					type: 'get',
					dataType: 'json',
					error: function(e){
...
//console.log(formData);
	$.ajax({
		url: "../ajax/empresas.php?op=guardaryeditar",
		type: "POST",
		data: formData,
		contentType: false,
		processData: false,
...
function mostrar(idempresa)
{
	...
	$.post("../ajax/empresas.php?op=mostrar",
			{idempresa : idempresa},
...
function desactivar(idempresa)
{
	bootbox.confirm("¿Estas seguro de desactivar el registro?", function(result)
	{
		if(result)
		{
			$.post("../ajax/empresas.php?op=desactivar",
					{idempresa : idempresa},
						function(e)
						{
							bootbox.alert(e);
...
function activar(idempresa)
{
	bootbox.confirm("¿Estas seguro de activar el registro?", function(result)
	{
		if(result)
		{
			$.post("../ajax/empresas.php?op=activar",
					{idempresa : idempresa},
						function(e)
						{
							bootbox.alert(e);
